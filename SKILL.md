...
description: "Use when asked for satellite imagery of a place, landmark, or coordinates. Newest clear Sentinel-2 L2A scene, free and keyless."
version: 1.0.0
author: Hermes Agent
license: MIT
...
metadata:
  hermes:
    tags: [satellite, imagery, sentinel-2, stac, earth-search, geospatial, cog, remote-sensing]
...
geocoding + Copernicus Sentinel-2 Level-2A via Element 84 Earth Search. No
account, no API key, no paid tier.

**Never imply this is live.** Sentinel-2 revisits every ~5 days and images are
published hours later. Say "acquired on <date>", not "live view".
...
```
<Place> — <lat>, <lon>
Acquired: <date, time UTC> (Sentinel-2<A|B|C>, tile <mgrs>) — <n> days old
...
Keep the attribution line — required, not narration.
...
$SK/.venv/bin/python -c "import rasterio, PIL, numpy, requests" 2>/dev/null || \
  $SK/.venv/bin/pip install rasterio pillow numpy requests
```
...
| Setting | Value |
|---|---|
| Source | Sentinel-2 L2A, collection `sentinel-2-c1-l2a` |
| Mode | `latest_clear` (≤20% local obscuration) |
| Search window | 60 days, auto-expanding to 120 then 365 |
| Radius | 15 km |
| Output | JPEG, ≤2048 px |

Literal recency wins over clearness: "most recent" → `--mode latest`. Explicit
date ranges are honored as given and never replaced by the 60-day default.
...
"Copernicus Sentinel-2 Level-2A imagery via Element 84 Earth Search." Endpoints:
`earth-search.aws.element84.com/v1` (STAC, no key), `geocoding-api.open-meteo.com`.
Asset hrefs are public S3 COGs; reads are byte-ranged, so a 10,980 px tile is
